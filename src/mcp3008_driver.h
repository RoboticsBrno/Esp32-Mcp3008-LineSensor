...
public:
    ...
    /**
     * \brief The Driver SPI configuration.
     */
    struct Config {
    ...
    Driver();
    virtual ~Driver(); //!< The uninstall() method is called from the destructor.
    ...
    /**
     * \brief Read values from the chip. Returns values in range <0; Driver::MAX_VAL>.
     *
